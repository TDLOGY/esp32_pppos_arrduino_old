...
static volatile bool rxDNS = false;
/**
 * DNS callback
 * @param name
 * @param ipaddr
 * @param callback_arg
 */
static void ssl_client_dns_found_callback(const char *name, const ip_addr_t *ipaddr, void *callback_arg)
{
    if(ipaddr) {
 ...
 *          else error code
 */
static int _hostByName(const char* aHostname, IPAddress& aResult)
{
    ip_addr_t addr;
    aResult = static_cast<uint32_t>(0);
    rxDNS = false;
    err_t err = dns_gethostbyname(aHostname, &addr, &ssl_client_dns_found_callback, &aResult);
...
#define ROE(x,msg) { if (((x)<0)) { log_e("LWIP Socket config of " msg " failed."); return -1; }}
    ...
    if ((flags = mbedtls_ssl_get_verify_result(&ssl_client->ssl_ctx)) != 0) {
        ...
        _stop_ssl_socket(ssl_client, rootCABuff, cli_cert, cli_key);  //It's not safe continue.
        return handle_error(ret);
    } else {
...
bool _verify_ssl_fingerprint(sslclient_context_t *ssl_client, const char* fp, const char* domain_name)
{
    ...
    if (domain_name)
        return _verify_ssl_dn(ssl_client, domain_name);
    else
        return true;
}
